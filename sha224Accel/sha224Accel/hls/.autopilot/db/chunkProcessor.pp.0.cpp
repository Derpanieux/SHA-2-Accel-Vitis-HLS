# 1 "../chunkProcessor/chunkProcessor.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 396 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../chunkProcessor/chunkProcessor.cpp" 2
...
# 2 "../chunkProcessor/chunkProcessor.cpp" 2

void chunkProcessor(word256 input[8], word256 message[16], word256 output[8]) {

    const static word256 kValues[64] = {
    ...
    word256 wValues[64];
    wGenerator(message, wValues);


    word256 wvars[8];
    VITIS_LOOP_22_1: for(int i = 0; i < 8; i++) {
        wvars[i] = input[i];
    }
    VITIS_LOOP_25_2: for(int i = 0; i < 64; i++) {
        word256 newwvars[8];
        chunkIter(kValues[i], wValues[i], wvars, newwvars);
        VITIS_LOOP_28_3: for (int j = 0; j < 8; j++) {
            wvars[j] = newwvars[j];
        }
    }
    VITIS_LOOP_32_4: for(int i = 0; i < 8; i++) {
        output[i] = input[i] + wvars[i];
    }
}
