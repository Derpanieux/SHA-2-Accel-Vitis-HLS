...
//
//==============================================================
`ifndef WGENERATOR_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define WGENERATOR_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class wGenerator_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "wGenerator_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(wGenerator_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(wGenerator_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            if (!uvm_config_db#(wGenerator_reference_model)::get(p_sequencer,"", "refm", refm))
            ...
            fork
                begin
                    fork
                        ...
                        begin
                            int delay;
                            for(int j=0; j<10000; j=j+refm.ap_done_cnt) begin
                                @refm.dut2tb_ap_done;
                ...
                begin
                    for(int j=0; j<10000; j=j+refm.ap_done_cnt) @refm.ap_done_for_nexttrans;
