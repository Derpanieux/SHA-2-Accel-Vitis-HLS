...
# 6 "./sha256Accel.h" 2
# 1 "./../chunkProcessor/chunkProcessor.h" 1



# 1 "./../chunkProcessor/../wGenerator/wGenerator.h" 1



# 1 "./../chunkProcessor/../wGenerator/../dataTypes.h" 1
...
# 5 "./../chunkProcessor/../wGenerator/../dataTypes.h" 2
...
# 5 "./../chunkProcessor/../wGenerator/wGenerator.h" 2
# 1 "./../chunkProcessor/../wGenerator/../functions256.h" 1
...
# 6 "./../chunkProcessor/../wGenerator/wGenerator.h" 2

void wGenerator(
    word256 win[16],
    word256 wout[64]
);
# 5 "./../chunkProcessor/chunkProcessor.h" 2
# 1 "./../chunkProcessor/../chunkIteration/chunkIter.h" 1
...
# 6 "./../chunkProcessor/chunkProcessor.h" 2

void chunkProcessor(word256 input[8], word256 message[16], word256 output[8]);
# 7 "./sha256Accel.h" 2

__attribute__((sdx_kernel("sha256Accel", 0))) void sha256Accel(hls::stream<bit> &bitstream, size256 size, hash256* output);
# 2 "sha256Accel.cpp" 2

__attribute__((sdx_kernel("sha256Accel", 0))) void sha256Accel(hls::stream<bit> &bitstream, size256 size, hash256* output) {
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=sha256Accel
# 3 "sha256Accel.cpp"
    ...
    VITIS_LOOP_10_1: while(iterneeded) {
        int j;
        VITIS_LOOP_12_2: for(j = 0; j < 448; j++, counter++) {
            if (counter < size) {
                bitstream >> buffer[j];
        ...
        if (!addSize) {
            VITIS_LOOP_23_3: for(; j < 512; j++, counter++) {
                if (counter < size) {
                    bitstream >> buffer[j];
                }else if (counter == size) {
                    buffer[j] = 1;
            ...
            int sizeIndex = 63;
            VITIS_LOOP_35_4: for(; j < 512; j++, counter++, sizeIndex--) {
        ...
        word256 message[16];
        int buffi = 0;
        VITIS_LOOP_43_5: for (int j = 0; j < 16; j++) {
            VITIS_LOOP_44_6: for (int k = 31; k >= 0; k--, buffi++) {
                message[j].set_bit(k, buffer[buffi]);
            }
        }
        chunkProcessor(interHash, message, wordsout);
        VITIS_LOOP_49_7: for(int j = 0; j < 8; j++) {
