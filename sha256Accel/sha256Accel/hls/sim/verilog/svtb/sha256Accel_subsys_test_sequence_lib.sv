...
//
//==============================================================
`ifndef SHA256ACCEL_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define SHA256ACCEL_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
    `define AUTOTB_TVIN_bitstream_bitstream_dout  "../tv/cdatafile/c.sha256Accel.autotvin_bitstream.dat" 
    `define AUTOTB_TVIN_size_size  "../tv/cdatafile/c.sha256Accel.autotvin_size.dat" 
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class sha256Accel_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "sha256Accel_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(sha256Accel_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(sha256Accel_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            svr_pkg::svr_slave_sequence #(256) svr_port_output_r_seq;            


            if (!uvm_config_db#(sha256Accel_reference_model)::get(p_sequencer,"", "refm", refm))
                ...
                begin
                    fork
                        begin
                            ...
                            if( refm.sha256Accel_cfg.port_bitstream_cfg.prt_type == AP_VLD ) wait(refm.misc_if.tb2dut_ap_start === 1'b1);
                        ...
                        begin
                            ...
                            if( refm.sha256Accel_cfg.port_size_cfg.prt_type == AP_VLD ) wait(refm.misc_if.tb2dut_ap_start === 1'b1);
